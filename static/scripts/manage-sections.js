...
function refreshtblData() {
    $.ajax({
        url: BASE_URL + 'crud/get_sections_details?s=' + se,
        method: 'GET',
        success: function (output, status, xhr) {
            ...
            if (return_obj.status == 1) {
                ...
                return_obj['data'].forEach(function (item, index, arr) {
                    t.row.add([
                        `<a href="javascript:;" onclick="get_edit_section('` + item[0] + `')" data-target="#modalEdit" data-toggle="modal"  title="Edit" class="mr-3"><i class="fas fa-edit"></i></a>
                        <a href="#" onclick="goto('`+ item[0] + `',1)" title="Manage Students" class="mr-3"><i class="fas fa-users"></i></a>`,
                        item[1],
                        item[2]
                    ]).draw();
                });
...
function refreshArchivedtblData() {
    $.ajax({
        url: BASE_URL + 'crud/get_archived_sections_details?s=' + se,
        method: 'GET',
        success: function (output, status, xhr) {
            const return_obj = JSON.parse(output);
            if (return_obj.status == 1) {
...
function get_edit_section(section) {
    $.ajax({
        url: BASE_URL + 'crud/get_edit_section?s=' + se,
        method: 'POST',
        data: { 's': section },
        success: function (output, status, xhr) {
            const return_obj = JSON.parse(output);

...
function refreshtblData() {
    $.ajax({
        url: BASE_URL + 'crud/get_sections_details_t?s=' + se,
        method: 'GET',
        success: function (output, status, xhr) {
            ...
            if (return_obj.status == 1) {
                ...
                return_obj['data'].forEach(function (item, index, arr) {
                    // <a href="javascript:;" onclick="get_edit_section('`+item[1]+`')" data-target="#modalEdit" data-toggle="modal"  title="Edit" class="mr-3"><i class="fas fa-edit"></i></a>
                    t.row.add([
